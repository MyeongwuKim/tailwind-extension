...
특히 margin, padding, font-size처럼 규칙이 정해져 있는 값들은
브라우저에서 얻은 실제 px 값과 미묘하게 어긋나는 경우가 많아
정확한 대응 기준을 잡는 것 자체가 쉽지 않은 상황이었습니다.

이런 이유로,
변환 가능한 속성만 선별해서 가져오고
Tailwind 스케일과 비교할 기준을 별도로 마련해야 했고,
이를 위해 별도의 JSON 기반 매칭 구조를 만들어 처리하는 방식으로 접근하게 되었습니다.
...
변환해서 만들어낸 Tailwind CSS를 웹페이지에 직접 삽입했을 때,
기존 페이지의 스타일이 Tailwind 규칙에 그대로 덮여서 화면이 깨지는 문제가 있었습니다.
